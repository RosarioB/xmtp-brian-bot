...
function extractCommandValues(text, commands) {
    if (!text.startsWith("/"))
...
function logInitMessage(client, config) {
...
class HandlerContext {
    ...
    static async loadCommandConfig(configPath = "skills.js") {
...
async function xmtpClient(config) {
    ...
    let key = getKey(config?.privateKey ?? process.env.KEY);
    ...
    //v2
    const account2 = accounts.privateKeyToAccount(key);
    const wallet2 = viem.createWalletClient({
        account: account2,
        chain: chains.mainnet,
        transport: viem.http(),
    });
...
function isPrivateKey(key) {
    if (key.length !== 66)
        return false;
    return accounts.privateKeyToAccount(key).address !== undefined;
}
...
async function run(handler, config) {
    ...
    const commandTriggered = (context) => {
        ...
        const isAddedMemberOrPass = typeId === "group_updated" &&
            ...
            ? false
            : true;
        const isRemoteAttachment = content?.contentType?.typeId == "remoteStaticAttachment";
        ...
        const isTagged = group && content?.startsWith("@") && skillGroup ? true : false;
    ...
    async function streamMessages(version) {
        ...
        while (true) {
            try {
                ...
                if (STREAM_LOG) {
                    console.error(`[${version}] Connection error (${connectionError?.code || "UNKNOWN"}). ` +
